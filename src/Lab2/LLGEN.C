/*--- llgen.c ------------------------------ Listing 2-3 --------
 *  Generic primitive functions 通用原始函数for doubly linked lists.
 *  Contains no application-specific functions.不包含特定于应用程序的功能。
 *  Functions are in alphabetical order.函数按字母顺序排列。
 *------------------------------------------------------------*/
 ...
 * 在头添加一个节点:首先为数据分配空间，然后用数据指针分配一个节点，然后将节点添加到列表中。
 *--------------------------------------------------*/
...
int AddNodeAscend ( struct List *L, void *nd )
{
    ...
    struct Node dummy;      /* a dummy node 一个虚拟节点*/
    int         compare;
 ...
 * 创建一个链表结构并返回一个指向它的指针。
   ...
   如果发生错误，返回NULL。这个函数接受指向四个特定于列表的函数的指针，
   并使用它们初始化链表结构。
 *-------------------------------------------------------------*/
 ...
 * 创建一个节点，然后调用特定于应用程序的函数CreateData()来创建节点的数据结构。
 * 如果发生错误，返回NULL。
 *-------------------------------------------------------------*/
 ...
 * 通过从列表的头部开始，逐步遍历每个节点，并与搜索键比较数据项来查找节点。
 * Ascend版本检查被检查节点中的数据是否大于搜索键。如果是，我们知道键不在列表中。
 * 如果成功返回节点指针，如果失败返回NULL。
 *-------------------------------------------------------------*/
