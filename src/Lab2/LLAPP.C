...
void *CreateData1(void *data) {
    ...
    if ((new_data = (NodeData1*)malloc ( sizeof ( struct NodeData1 ))) == NULL)
        return (NULL);
...
int DuplicatedNode1(Link new_node, Link list_node) {            /* adding an occurrence to an existing word */
...
/*=== Now the functions for the second linked list 现在是第二个链表的函数===*/

void *CreateData2(void *data) {
    struct NodeData2 * new_data;

    /*--- allocate the data structure ---*/
    if ((new_data = (NodeData2*)malloc ( sizeof ( struct NodeData2 ))) == NULL)
        return ( NULL );
